...
model.mf_CA = Var(within=PositiveReals, bounds=(0.5,2), initialize = 1)         #5.5
model.ms_CA = Var(within=PositiveReals, bounds=(0.9,2), initialize = 1)         #3.8 
model.md_CA = Var(within=PositiveReals, bounds=(0.5,2), initialize = 1)         #1.3 
...
for s in Sample:
    if np.any(CA_meas[s]==0):
        print('There is 0 acid data in the training set')
...
for s in Sample_test:
    if np.any(CA_test[s]==0):
        print('There is 0 acid in testing dataset')
