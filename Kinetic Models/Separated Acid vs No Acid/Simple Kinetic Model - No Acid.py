...
model.Ah = Var(within=PositiveReals, bounds=(10000, 1e30), initialize=8e14)
...
for s in Sample:
    if np.any(CA_meas[s]==0):
        model.constraints.add(log(model.kh[s]) == log(model.Ah) - model.Eh/(R*Tem_meas[s]) )
        model.constraints.add(log(model.kd[s]) == log(model.Ad) - model.Ed/(R*Tem_meas[s]) ) 
    else:
        print("There are acid datapoints in the training dataset")
...
for s in Sample_test:
    if np.any(CA_test[s]==0):
        model_test.constraints.add(log(model_test.kf_test[s]) == log(bestAh) - bestEh/(R*Tem_test[s]) )
        model_test.constraints.add(log(model_test.kd_test[s]) == log(bestAd) - bestEd/(R*Tem_test[s]) )
    else:
        print("There are acid datapoints within the testing dataset")
